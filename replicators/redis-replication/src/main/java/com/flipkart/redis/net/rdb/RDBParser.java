...
public class RDBParser {
    ...
    /* Redis的特殊标示 */
    public static final int REDIS_EXPIRETIME_FC = 252; /* Millisecond time expired, occupies 8 bytes */

    public static final int REDIS_EXPIRETIME_FD = 253;	/* Second grade expiration time, occupy 8 bytes */

    public static final int REDIS_SELECTDB = 254;	/* Database (database is immediately followed by number) */

    public static final int REDIS_EOF = 255;	/* terminator */
    
    /*
     * It represents the length of mode
     */
    ...
    /* Redis file header, must begin with "REDIS" string **/
    ...
    private boolean readBytes(byte[] buf, int start, int num) {
        ...
        int nget = Math.min((int)(streamLength - bytesRead), num);
        
        int offset = 0;
		while (offset < nget) {
			final int size = inputStream.read(buf, start + offset, (nget - offset));
			if (size == -1)
    ...
    /**
     * The two byte arrays are compared to determine whether equal
     * @param buf1
     * @param start1
     * @param buf2
     * @param start2
    ...
    /* Get the expiration time, byte array must be converted to Long */
    ...
    public void init(RedisInputStream is) {
    	...
    	this.inputStream = is;
    	this.bytesRead = 0;
    	this.streamLength = is.readLongCrLf();
