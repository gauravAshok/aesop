...
public class KeyUpdateObservableMapper {
	...
	public Observable<Reply<KeyTypePair>> map(Reply<List<String>> cmd) {
		...
		if(txnBegin == false) {
			...
			return Observable.from(keysUpdated.stream().map(str -> new Reply<KeyTypePair>(new KeyTypePair(str, command.datatype), bytesReadSinceLastUpdate))
	...
	private Observable<Reply<KeyTypePair>> endTxn() {
		...
		for(String key : keysModifiedInCurrentTxn.keySet()) {
			updatedKeys.add(new Reply<KeyTypePair>(
					new KeyTypePair(key, keysModifiedInCurrentTxn.get(key)), bytesReadInCurrentTxn));
		}
